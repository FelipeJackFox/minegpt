...
> 1. This file
> 2. `../pipeline/HARDENING_V2_RESULTS.md` (final pipeline state)
> 3. `../pipeline/QA_GENERATION_PLAN.md` (next phase)
> 4. `C:/Users/luis/.claude/projects/D--Code-minegpt/memory/MEMORY.md` (memory index)
>
> Replaces `../archive/SESSION_HANDOFF_2026-04-27.md`.

---

## TL;DR — current state

1. **Hardening v2 pipeline is DONE and applied.** Corpus split into 7,135 main + 2,834 qa_direct + 174 dropped. 9.27M → 7.62M words (17.77% loss). Idempotent. Source: `scraper/hardening_v2.py`. Final state in `../pipeline/HARDENING_V2_RESULTS.md`.
...
- Updated 4 partially-stale docs (decision 2026-05-03 was to materialize the updates, not just banner them):
  - `pipeline/WIKI_DATA_CLEANING.md` v1 → archived; replaced by slim `pipeline/PIPELINE_OVERVIEW.md`
  - `pipeline/HARDENING_V2_PLAN.md` → archived (spec we worked from; `HARDENING_V2_RESULTS.md` is canonical)
  - `prompts/PROMPT_TEMPLATES.md` v1 (Transform + Q&A) → archived; replaced by Q&A-only successor
  - `prompts/PHASE4_TRANSFORMATION_PLAN.md` → archived (transform deprecated; Q&A halves consolidated)
...
### New docs
- `../pipeline/HARDENING_V2_RESULTS.md` — final state of hardening pipeline + iteration log
- `../pipeline/QA_GENERATION_PLAN.md` — consolidated Q&A plan replacing transformation
- `../tools/PROMPT_LAB_UI.md` — current UI architecture, replaces archived PHASE4_UX_DESIGN
...
1. **Iterate Q&A prompts** in Prompt Lab. Pilot bucket suggestion: `Animal_mobs` (47 articles, dense facts, validates flow). See `../pipeline/QA_GENERATION_PLAN.md` "Status checklist".
...
5. **Tokenize + train v1 model**. Decide model architecture (125-200M target). See `../pipeline/PIPELINE_OVERVIEW.md` § Phase 6 + `../archive/WIKI_DATA_CLEANING_v1.md` post-pipeline section for original architecture-choice notes (the rest of that doc is deprecated).
